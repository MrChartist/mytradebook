          ...
          <div className="flex min-h-0">
            ...
            <div className="hidden sm:flex flex-col w-16 border-r border-border/15 bg-muted/5 py-2.5 gap-0.5 items-center shrink-0">
              {sidebarItems.map((item, i) => {
                ...
                return (
                  <div key={i} className={cn("w-9 h-9 rounded-lg flex items-center justify-center transition-colors relative", active ? "bg-primary/10 text-primary" : "text-muted-foreground/30 hover:text-muted-foreground/50")}>
            ...
            <div className="flex-1 p-3 sm:p-5 min-w-0">
              ...
              <div className="flex items-start justify-between mb-2 sm:mb-3">
                ...
                <div className="hidden sm:flex items-center gap-0.5 bg-muted/40 rounded-full p-0.5">
                  {["Jan", "Feb", "Mar"].map((m, i) => (
                    <span key={m} className={cn("px-2.5 py-1 text-[9px] font-medium rounded-full transition-colors", i === 2 ? "bg-background shadow-sm text-foreground" : "text-muted-foreground")}>
                      {m}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex gap-1 sm:gap-1.5 mb-3 sm:mb-4 flex-wrap">
                {["All", "Intraday", "Positional", "Futures", "Options", "Commodities"].map((s, i) => (
                  <span key={s} className={cn("px-2 sm:px-2.5 py-0.5 sm:py-1 text-[8px] sm:text-[9px] font-medium rounded-full border transition-colors", i === 0 ? "bg-primary text-primary-foreground border-primary shadow-[0_0_8px_hsl(var(--tb-accent)/0.3)]" : "border-border/40 text-muted-foreground")}>
                    {s}
                  </span>
                ))}
              </div>
                ...
                <div className="sm:col-span-3 rounded-xl border border-border/20 bg-card p-2.5 sm:p-3" style={{ boxShadow: "inset 0 1px 0 0 hsl(0 0% 100% / 0.04)" }}>
                  <div className="flex items-center justify-between mb-2 sm:mb-3">
                    ...
                    <div className="flex gap-0.5 bg-muted/40 rounded-full p-0.5">
                      {["1W", "1M", "3M"].map((t, i) => (
                        <span key={t} className={cn("px-1.5 sm:px-2 py-0.5 text-[7px] sm:text-[8px] font-medium rounded-full transition-colors", i === 1 ? "bg-background shadow-sm text-foreground" : "text-muted-foreground")}>
                          {t}
                        </span>
                      ))}
                    </div>
