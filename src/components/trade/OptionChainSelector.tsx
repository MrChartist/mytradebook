...
function generateExpiryDates(): string[] {
  ...
  // If today is Thursday (4), go to next week's Thursday
...
export function OptionChainSelector({ onSelect, className }: OptionChainSelectorProps) {
  ...
  // Memoize expiries to avoid recalculating on every render
  ...
  // Generate option LTP (mock) - use useCallback to prevent infinite loops
  const getOptionLtp = useCallback((strikePrice: number, type: "CE" | "PE"): number => {
    if (!atmPrice) return 0;
    const diff = type === "CE" ? atmPrice - strikePrice : strikePrice - atmPrice;
    const intrinsic = Math.max(0, diff);
    // Use deterministic time value based on strike to avoid random regeneration
  ...
  }, [strike, optionType, getOptionLtp]);
