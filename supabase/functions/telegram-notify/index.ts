...
const conditionHeaders: Record<string, string> = {
  ...
  PRICE_CROSS_ABOVE: "CROSS ABOVE ⚡",
  PRICE_CROSS_BELOW: "CROSS BELOW ⚡",
  PERCENT_CHANGE_GT: "DAY % ABOVE",
  PERCENT_CHANGE_LT: "DAY % BELOW",
...
async function sendTelegramPhoto(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { success: false, error: errorDescription, errorCode, errorDescription, retryable };
    }
    return { success: true, messageId: result.result?.message_id };
  } catch (e) {
    console.error("Failed to send Telegram photo:", e);
    return { success: false, error: e instanceof Error ? e.message : "Network error", retryable: true };
  }
}
...
async function sendToMultipleChats(
  ...
  for (const chat of chats) {
    ...
    if (result.success) {
      successCount++;
      results.push({ chat_id: chat.chat_id, success: true, message_id: result.messageId, token_tier: tokenTier });
    } else {
      failCount++;
      results.push({ chat_id: chat.chat_id, success: false, error: result.error || "Unknown error", token_tier: tokenTier });
    }
  }
...
// ════════════════════════════════════════════════════════════════════
//  PHASE 1 & 2: REFINED MESSAGE TEMPLATES
// ════════════════════════════════════════════════════════════════════
...
function buildAlertTriggeredMessage(alert: any, ltp: number): string {
  ...
  const triggerCount = (alert.trigger_count || 0) + 1;

  // Calculate distance metrics
  let distanceText = "";
  if (!isPct && !isVol && alert.threshold && ltp > 0) {
    const distance = ((ltp - alert.threshold) / alert.threshold) * 100;
    distanceText = `Distance: ${distance >= 0 ? "+" : ""}${distance.toFixed(2)}% from level`;
  }

  // Build trigger text
  let triggerText = "";
  if (isPct) triggerText = `Day % ${ct.includes("GT") ? "above" : "below"} ${alert.threshold}%`;
  else if (isVol) triggerText = `Volume spike triggered`;
  ...
  const intervalMin = alert.check_interval_minutes || 5;
  const intervalLabel = intervalMin >= 60 ? `${Math.floor(intervalMin / 60)}h` : `${intervalMin}m`;
  ...
  msg += `✅ ${triggerText}\n`;
  msg += `💰 LTP: *${fmt(ltp)}*\n`;
  if (distanceText) msg += `📏 ${distanceText}\n`;
  msg += `\n${SEPARATOR}\n\n`;
  msg += `⚙️ Mode: ${mode}\n`;
  msg += `🔄 Check: every ${intervalLabel}\n`;
  if (triggerCount > 1) msg += `📊 Trigger #${triggerCount}\n`;
  if (alert.notes) msg += `\n📝 _${alert.notes}_\n`;
  if (alert.chart_link) msg += `\n📊 [View Chart](${alert.chart_link})\n`;
  msg += `\n⏱️ ${time} IST`;
  msg += FOOTER;
  return msg;
}
...
function buildAlertPausedMessage(alert: any, isPaused: boolean): string {
  ...
  if (isPaused) {
    return `⏸️ *ALERT PAUSED*\n\n📍 ${alert.symbol} \`${tag}\`\n${condLabel} ${triggerText}${FOOTER}`;
  }
  return `▶️ *ALERT RESUMED*\n\n📍 ${alert.symbol} \`${tag}\`\n${condLabel} ${triggerText}${FOOTER}`;
}
...
function buildNewTradeMessage(trade: any, isRaMode: boolean, disclaimer: string | null): string {
  ...
  let slLine = trade.stop_loss ? `🛑 SL: ${fmt(trade.stop_loss)}` : "🛑 SL: —";
  if (trade.stop_loss && trade.entry_price) {
    const slPct = ((trade.entry_price - trade.stop_loss) / trade.entry_price * 100 * (side === "BUY" ? 1 : -1));
    if (!Number.isNaN(slPct)) slLine += ` (${Math.abs(slPct).toFixed(1)}%)`;
  }
  ...
  if (segment === "Options") {
    riskNote = side === "SELL" 
      ? "\n\n⚠️ _Option selling needs defined risk._" 
      : "\n\n⚠️ _Options are high risk._";
  }

  let msg = `${sideEmoji} *NEW ${side} TRADE*\n`;
  msg += `${SEPARATOR}\n\n`;
  msg += `📍 *${trade.symbol}* \`${tag}\`\n`;
  msg += `📂 ${label}\n\n`;
  msg += `💰 Entry: ${trade.entry_price ? fmt(trade.entry_price) : "At LTP"}\n`;
  msg += `${slLine}\n`;
  ...
  if (tf) msg += ` | TF: ${tf}`;
  msg += "\n";
  if (tslLine) msg += `${tslLine}\n`;
  if (trade.notes) msg += `\n📝 _${trade.notes}_\n`;
  if (trade.chart_link) msg += `\n📊 [View Chart](${trade.chart_link})\n`;
  msg += riskNote;
  if (isRaMode) {
    msg += "\n\n⚠️ _Position sizing as per your risk plan._";
    if (disclaimer) msg += `\n\n🧾 _${disclaimer}_`;
  }
  msg += FOOTER;
  return msg;
...
function buildTradeClosedMessage(trade: any, isRaMode: boolean, disclaimer: string | null): string {
  ...
  if (!isRaMode) msg += `\n${emoji} *P&L: ${fmt(pnl)} (${fmtPct(trade.pnl_percent)})*\n`;
  msg += `\n${SEPARATOR}\n\n`;
  msg += `🏁 Outcome: *${outcome}*\n`;
...
function buildTradeUpdateMessage(trade: any, latestEvent: any, isRaMode: boolean): string {
  ...
  if (eventType.includes("TARGET") && eventType !== "TARGET_MODIFIED") {
    ...
    let msg = `🎯 *TARGET HIT*\n${SEPARATOR}\n\n`;
    msg += `📍 *${symbol}*\n\n`;
    msg += `✅ ${targetNum} achieved!\n`;
    msg += `💰 LTP: ${fmt(trade.current_price)}\n`;
    if (!isRaMode) msg += `${emoji} P&L: ${fmt(pnl)} (${fmtPct(trade.pnl_percent)})\n`;
    msg += `\n💡 _Consider partial booking + trail SL_`;
    if (trade.trailing_sl_current) msg += `\n🔄 TSL: ${fmt(trade.trailing_sl_current)}`;
    msg += FOOTER;
    return msg;
  }
  ...
  if (eventType === "SL_HIT") {
    ...
    if (!isRaMode) msg += `\n${emoji} P&L: ${fmt(pnl)} (${fmtPct(trade.pnl_percent)})`;
    if (latestEvent?.notes) msg += `\n\n📝 _${latestEvent.notes}_`;
    msg += `\n\n💡 _Wait for fresh setup._`;
    msg += FOOTER;
    return msg;
  }
  ...
  // Generic update
  let msg = `📊 *TRADE UPDATE*\n${SEPARATOR}\n\n`;
  msg += `📍 *${symbol}*\n`;
  msg += `📋 Event: ${eventType.replace(/_/g, " ")}\n\n`;
  msg += `💰 LTP: ${fmt(trade.current_price)}\n`;
  if (!isRaMode) msg += `${emoji} P&L: ${fmt(pnl)} (${fmtPct(trade.pnl_percent)})\n`;
...
function buildSLModifiedMessage(trade: any, oldSL: number, newSL: number): string {
  ...
  let msg = `🔄 *SL TRAIL*\n${SEPARATOR}\n\n`;
  msg += `📍 *${trade.symbol}*\n\n`;
  msg += `📉 Old SL: ${fmt(oldSL)}\n`;
  msg += `📈 New SL: ${fmt(newSL)}\n`;
  msg += `\n${emoji} Current P&L: ${fmt(pnl)} (${fmtPct(trade.pnl_percent)})`;
  msg += FOOTER;
  return msg;
}
...
function buildManualTradeSnapshotMessage(trade: any, isRaMode: boolean, disclaimer: string | null): string {
  ...
  let msg = `📋 *TRADE SNAPSHOT*\n${SEPARATOR}\n\n`;
  msg += `${sideEmoji} *${side} ${trade.symbol}* \`${tag}\`\n`;
  msg += `📂 ${label}\n\n`;
  msg += `💰 Entry: ${fmt(trade.entry_price)}\n`;
  msg += `💰 LTP: ${fmt(trade.current_price)}\n`;
  msg += `🛑 SL: ${fmt(trade.stop_loss)}\n`;
...
function buildManualCustomNoteMessage(trade: any, customMessage: string): string {
  ...
  msg += `\n${SEPARATOR}\n\n`;
  msg += `💰 LTP: ${fmt(trade.current_price)}\n`;
  msg += `${emoji} P&L: ${fmt(pnl)} (${fmtPct(trade.pnl_percent)})`;
  msg += FOOTER;
  return msg;
}
...
function buildStudyMessage(study: any, action: string, oldStatus?: string): string {
  ...
  if (attachments.length > 0) {
    ...
    if (imgUrl) msg += `🖼️ [View Image](${imgUrl})\n`;
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // For "custom" with explicit chat_id — direct send
    ...
    switch (payload.type) {
      ...
      case "weekly_report": {
        ...
        message += `📂 ${report.segment.replace("_", " ")}\n`;
        message += `📅 ${report.week_start} to ${report.week_end}\n\n`;
        message += `${emoji} *Net P&L: ${fmt(totalPnl)}*\n\n`;
        message += `📈 Total Trades: ${fmtNum(report.total_trades)}\n`;
        message += `🎯 Win Rate: ${fmtPct(report.win_rate)}\n`;
        message += `✅ Best: ${fmt(report.best_trade_pnl)}\n`;
        message += `❌ Worst: ${fmt(report.worst_trade_pnl)}\n`;
        message += `\n${SEPARATOR}\n\n`;
        message += `🎯 *Top Setups*\n${topSetupsStr}\n\n`;
        message += `⚠️ *Common Mistakes*\n${mistakesStr}`;
        message += FOOTER;
        break;
      }
      case "study_created": {
